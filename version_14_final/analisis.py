...
def p_Program(p):
    ...
    # Aquí se corrige el error de la lista, devolviendo un objeto ASTNode
    p[0] = Program(p[1])
...
def p_CaseItem(p):
    """CaseItem : CASE INTLIT ':' Statements
                | DEFAULT ':' Statements"""
    # Si es "case", las sentencias están en p[4]
    if p[1] == 'case': 
        stmts = Block([], p[4])
        p[0] = (p[2], stmts)
    # Si es "default", las sentencias están en p[3]
    else: 
        stmts = Block([], p[3])
        p[0] = ('default', stmts)
...
# --- 3. GENERADOR DE CÓDIGO INTERMEDIO (LLVM IR) ---
